...
def checkout(request):
    ...
    # Формируем текстовый чек для вывода в консоль
    print("\n" + "="*60)
    print(" НОВЫЙ ЗАКАЗ ОФОРМЛЕН")
    print("="*60)
    print(f"👤 Покупатель: {request.user.username}")
    print(f"📍 Адрес доставки: {user_address}")
    print(f"💬 Комментарий: {user_comment}")
    print("-"*60)
    print(f"{'Наименование товара':<30} {'Кол-во':<8} {'Цена':<10} {'Итого':<10}")
    print("-"*60)
    ...
    for item in cart_items:
        ...
        print(f"{item.product.title:<30} {item.quantity:<8} {item.product.price:<10} {item_total:<10}")
    ...
    return HttpResponse(
        ...
        f"<h2 style='color: #28a745;'>🎉 Заказ успешно оформлен!</h2>"
        f"<p><b>Покупатель:</b> {request.user.username}</p>"
        f"<p><b>Адрес доставки:</b> {user_address}</p>"
        f"<p><b>Комментарий:</b> {user_comment}</p>"
        f"<hr>"
        f"<p style='font-size: 24px;'><b>Итого к оплате: {total_price} руб.</b></p>"
        f"<p style='color: #666;'><i>Подробный чек выведен в консоль сервера.</i></p>"
        f"<a href='/catalog/' style='display: inline-block; background: #007bff; color: white; padding: 10px 15px; text-decoration: none; border-radius: 4px; margin-top: 20px;'>Вернуться в каталог</a>"
        f"</div></body>"
    )
